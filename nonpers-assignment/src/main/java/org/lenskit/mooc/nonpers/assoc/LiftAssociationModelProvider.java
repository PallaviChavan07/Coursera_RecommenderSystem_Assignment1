...
public class LiftAssociationModelProvider implements Provider<AssociationModel> {
    ...
    public AssociationModel get() {
        ...
        double allUsersCount = allUsers.size();
        ...
        for (Long2ObjectMap.Entry<LongSortedSet> xEntry: itemUsers.long2ObjectEntrySet()) {
            ...
            double xUsersCount = xUsers.size();
            ...
            for(Long2ObjectMap.Entry<LongSortedSet> yEntry : itemUsers.long2ObjectEntrySet()){
                ...
                double yUsersCount = yUsers.size();


                //yandXIntersect.retainAll(yUsers);
                long yandXIntersect = 0;
                for(Long xUser : xUsers){
                    if(yUsers.contains(xUser)){
                        yandXIntersect++;
                    }
                }
                double probOfYandX = yandXIntersect / allUsersCount;
                double probOfX = xUsersCount / allUsersCount;
                double probOfY = yUsersCount / allUsersCount;
                itemScores.put(yId, probOfYandX/(probOfX*probOfY));
